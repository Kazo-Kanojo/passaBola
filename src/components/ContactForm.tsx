...
const ContactForm = () => {
  ...
  const validate = () => {
    ...
    if (!formData.email) tempErrors.email = 'Email é obrigatório';else if (!/\S+@\S+\.\S+/.test(formData.email)) tempErrors.email = 'Email inválido';
  ...
  return <section id="contato" className="py-20 bg-gradient-to-b from-primary/5 to-tertiary/5">
        ...
        <div className="max-w-2xl mx-auto">
          <form onSubmit={handleSubmit} className="card p-8 fade-in">
            ...
            <div className="mb-6">
              ...
              <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} className={`w-full px-4 py-3 rounded-lg border ${errors.name ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-primary`} placeholder="Seu nome" />
            ...
            <div className="mb-6">
              ...
              <textarea id="message" name="message" value={formData.message} onChange={handleChange} rows={5} className={`w-full px-4 py-3 rounded-lg border ${errors.message ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-primary`} placeholder="Digite sua mensagem aqui..."></textarea>
              {errors.message && <p className="text-red-500 text-sm mt-1">{errors.message}</p>}
            </div>
            <button type="submit" disabled={isSubmitting} className={`btn-primary w-full flex items-center justify-center ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}>
